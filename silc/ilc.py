...
class ILC_simple:
    def __init__(self, Cosmology, fgs ,fwhms=[1.5], rms_noises=[1.], freqs=[150.], lmax=8000, lknee=0., alpha=1., dell=1., v3mode=-1, fsky=None, noatm=False, add='None',fun=1):
        '''
        Inputs
        ...
        if v3mode>-1:
            ...
            if add=='P':
                ...
                experimentName2 = 'LiteBird'
                
                beams2 = list_from_config(Config,experimentName2,'beams')
                noises2 = list_from_config(Config,experimentName2,'noises')
                freqs2 = list_from_config(Config,experimentName2,'freqs')
                lmax2 = int(Config.getfloat(experimentName2,'lmax'))
                lknee2 = list_from_config(Config,experimentName2,'lknee')[0]
                alpha2 = list_from_config(Config,experimentName2,'alpha')[0]
                fsky2 = 0.7
...
#                freqs_nosat = freq
                ...
                #_noatm
                N_ell_T_LA_C = np.diagonal(N_ell_T_LA_full_C).T
                #Map_white_noise_levels = lat.get_white_noise(fsky=0.4)**.5
...
#            else:
            ...
            if add =='P':
                ...
                l_lmax=4000
                if self.evalells[ii] < l_lmax:
                    inst_noise2 = noise_func(self.evalells[ii],np.array(beams2),np.array(noises2),lknee2,alpha2,dimensionless=False)/self.cc.c['TCMBmuK']**2.
                    inst_noise2_pol = np.sqrt(2.)* inst_noise2
            ...
            self.N.append(np.diagonal(nells* self.cc.c['TCMBmuK']**2.))
            self.NP.append(np.diagonal(nells_pol* self.cc.c['TCMBmuK']**2.))
            ...
            fgrspol=np.append(fgrspol,
                              (self.fgs.rs_autoEE(self.evalells[ii],freqs[None,:],freqs[:,None])+self.fgs.gal_sync_pol_new(self.evalells[ii],freqs[None,:],freqs[:,None])+self.fgs.gal_dust_pol_new(self.evalells[ii],freqs[None,:],freqs[:,None])+self.fgs.rad_pol_ps_new(self.evalells[ii],freqs[None,:],freqs[:,None]))/ self.cc.c['TCMBmuK']**2. / ((self.evalells[ii]+1.)*self.evalells[ii]) * 2.* np.pi)
            ...
            cib = np.append(cib,
                            (self.fgs.cib_p(self.evalells[ii],freqs[None,:],freqs[:,None]) + self.fgs.cib_c(self.evalells[ii],freqs[None,:],freqs[:,None])) \
                                / self.cc.c['TCMBmuK']**2. / ((self.evalells[ii]+1.)*self.evalells[ii]) * 2.* np.pi)
            rps=np.append(rps,(self.fgs.rad_ps(self.evalells[ii],freqs[None,:],freqs[:,None])) \
                                      / self.cc.c['TCMBmuK']**2. / ((self.evalells[ii]+1.)*self.evalells[ii]) * 2.* np.pi)
            tszcib=np.append(tszcib,self.fgs.tSZ_CIB(self.evalells[ii],freqs[None,:],freqs[:,None])\
                                      / self.cc.c['TCMBmuK']**2. / ((self.evalells[ii]+1.)*self.evalells[ii]) * 2.* np.pi)
        ...
        self.rps = np.reshape(rps,[len(self.evalells),len(self.freqs),len(self.freqs)])
        self.tszcib = np.reshape(tszcib,[len(self.evalells),len(self.freqs),len(self.freqs)])
    ...
    def bias(self,constraint=False):
    ...
    def cross_err_calc(self,ell,C1,N1,C2,N2,X,detect=True):
        ...
        s2n=np.sqrt(s2n)
        return s2n,errs
    def auto_err_calc(self,ell,C1,N1):
        covs = []
        s2n=[]
        
        for k in range(len(ell)):
            i=int(ell[k])
            ClSum = np.nan_to_num((C1[k]+N1[k])**2)
            #if (detect):
             #   s2nper=(2*i+1)*np.nan_to_num((C1[i]**2)/(N1[k]**2))
            #else:
            s2nper=(2*i+1)*np.nan_to_num((C1[k]**2)/(C1[k]+N1[k])**2)
            var = 2*ClSum/(2.*i+1.)/self.fsky
            covs.append(var)
            s2n.append(s2nper)

        errs=np.sqrt(np.array(covs))
        #s2n.sort(reverse=True)
        s2n=self.fsky*sum(s2n)/2
        s2n=np.sqrt(s2n)
        return s2n,errs
    ...
    def tsz_forecast(self,ellmax,constrained=None):
        fsky = self.fsky
        ells = np.arange(2,ellmax,1)
        Nll_tsz = self.tsz_opt(constrained='cib')
        cls=self.fgs.tSZ(self.evalells,self.fgs.nu_rs,self.fgs.nu_rs) / self.cc.c['TCMBmuK']**2.\
            / ((self.evalells+1.)*self.evalells) * 2.* np.pi
        sn,nlls=self.auto_err_calc(ells,cls,Nll_tsz)
        return cls,nlls,sn
