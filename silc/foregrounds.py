...
class fgNoises(object):
    ...
    def __init__(self,constDict,ksz_file=root_dir+'input/ksz_BBPS.txt',ksz_p_file=root_dir+'input/ksz_p_BBPS.txt',tsz_cib_file=root_dir+'input/sz_x_cib_template.txt',ksz_battaglia_test_csv=None,tsz_battaglia_template_csv=root_dir+"input/sz_template_battaglia.csv",rs_template=root_dir+"data/fiducial_scalCls_lensed_7_7.txt",rsx_template=root_dir+"data/fiducial_scalCls_lensed_1_7.txt",components=None,lmax=None):
        self.c = constDict
        ...
        self.nu_rs = 280.#145 ## hard coded from template
...
# =========== converting/blackbody functions ===========
    ...
    beta_d = 1.59
    beta_s=-2.91
    def pl_dust_power(self,freq_in_GHz, nu_353=364., beta_d=1.59):
        """
        in RJ
        """
        nu = freq_in_GHz
    ...
    def gal_dust_pol(self,ell,nu1,nu2):
        ...
        in RJ
        """
        mu1 = self.pl_dust_power(nu1)
        mu2 = self.pl_dust_power(nu2)
        ans =  mu1 * mu2*(ell/80.)**(-0.42)#(old_div(ell,self.c['ell0sec'])) ** (-2.42)*ell*(ell+1)/self.c['ell0sec']/(self.c['ell0sec']+1) *  #/ mu0**2
        return ans
    ...
    def gal_dust_pol_new(self,ell,nu1,nu2):
        ...
        in CMB
        """
        x=0.53*self.gal_dust_pol(ell,nu1,nu2)*self.A_to_CMB(nu1)* self.A_to_CMB(nu2)#/(self.A_to_CMB(364.))**2#check constant
        return x
    ...
    def gal_dust_pol_old(self,ell,nu1,nu2):
        ...
        return ans
    #OLD
    def gal_sync_pol_old(self,ell,nu1,nu2):
        ...
        return ans
    #OLD
    def rad_pol_ps_old(self,ell,nu1,nu2):
